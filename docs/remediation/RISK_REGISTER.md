...
## Synthetic Integration Lab Sprint B Status Update

| ID | Status | Evidence | Residual Risk |
|---|---|---|---|
| LAB-ISO-001 | Extended for offline core routes | Sprint B routes remain under `/lab/*`, require central guard, bearer auth, lab session id/token, route-class rate limit, and feature-specific default-false flags. | Review branch only; not merged to `dev`; browser-rendered QA and Sprint C red-team remain pending. |
| LAB-FIX-001 | Extended for generated core fixtures | Manifest now covers generated synthetic-only cases, RAG, registry, EBP, uploads, rendering, image/mock, and expected-output fixtures. Closure review adds duplicate path/id rejection, category and `.json` enforcement, size/depth bounds, all-fixture manifest coverage checks, and real registry/data path rejection. | Fixture reviews must continue for every future corpus expansion; licensed registry, hospital, and patient content remain forbidden. |
| LAB-TRC-001 | Extended for owner/session trace binding | Trace creation binds records out-of-band to the creating principal and lab session id; trace reads require the same bearer principal, lab session id, and lab session token. Tests cover same-session access, cross-session denial, cross-principal denial, missing/wrong token denial, expired trace denial, random id denial, and `run_id`-alone denial. | Trace store remains in-memory only; new trace fields still require explicit allowlist review and canary tests. |
| LAB-AGT-001 | Partially mitigated for deterministic lab showcase | `/lab/run` invokes separate deterministic stages and records safe stage metadata; critic behavior affects synthesis; stage failure/timeout abstain. | This is a multi-stage agent orchestration prototype only, not autonomous swarm coordination, not clinical swarm intelligence, and not real-provider orchestration. |
| LAB-RAG-001 | Partially mitigated for synthetic lexical RAG showcase | `/lab/rag` uses generated offline corpus fixtures, normalized token overlap, bounded top-k, stable ids/scores, and weak-context abstention. | No embeddings, vector database, hybrid retrieval, reranking, current EBP evidence, or validated clinical RAG is implemented. |
| LAB-REG-001 | Partially mitigated for lab-only fake-code registry | `/lab/registry` loads manifest-approved fake codes such as `SYN-D-001`; responses report `registry_authoritative=false`, `clinical_use_allowed=false`, `accepted_recommendations=false`, and `nurse_review_required=true`; normal code-format policy rejects `SYN-D-001`. | No official registry grounding, release approval, licensed content, or durable authoritative registry store is implemented. |
| LAB-EBP-001 | Partially mitigated for offline fixture adapter | `/lab/ebp` returns generated offline fixture references only and network-deny tests patch external EBP connectors to fail. | Offline fixture is not current medical evidence and must not be presented as external evidence retrieval. |
| LAB-OCR-001 | Partially mitigated for explicit mock label | `/lab/ocr` returns `DETERMINISTIC OCR MOCK - EXTRACTION UNVERIFIED` from fixture metadata only. | No OCR SDK, clinical OCR validation, image text extraction, or clinical interpretation exists. |
| LAB-IMG-001 | Partially mitigated for explicit mock label | `/lab/photo` returns `DETERMINISTIC PHOTO MOCK - NOT CLINICAL IMAGE ANALYSIS` from fixture metadata only. | No clinical photo analysis or image model exists; normal photo capability remains unavailable. |
| LAB-MEM-001 | Partially mitigated for lab-only feedback namespace | `/lab/feedback` uses a bounded in-memory lab store, rejects PHI/secret canaries, and does not cross sessions or normal memory. | No durable memory, authoritative learning, registry update, or production feedback governance is implemented. |
| LAB-AUD-001 | Extended with safe lab audit metadata | Lab event types are allowlisted and audit metadata is limited to run id, route, fixture id, feature label, principal fingerprint, status, and timestamps. | HMAC ledger residuals remain: not WORM, not immutable storage, not non-repudiation, and no external anchor. |
| LAB-UI-001 | Extended with safe trace panel | Frontend source tests verify safe labels, server-authoritative gating, lab trace headers, no storage persistence, and no raw prompt/output/upload/path/stack/chain-of-thought fields. | Source-level tests are not browser-rendered QA; Sprint C must verify rendered banner/panel behavior. |

## Synthetic Integration Lab Sprint B Residuals

| Risk | Severity | Status | Next Action |
|---|---|---|---|
| Offline synthetic showcase can be mistaken for clinical capability | High | Labels and docs state prototype/mock/non-authoritative status, but UI polish can still invite overtrust. | Sprint C must add browser QA, demo script, and known-limitations runbook before any showcase claim. |
| External-provider and EBP internet remain disabled | High | Network-deny tests pass and config flags remain false by default. | Keep provider work on a separate `review/synthetic-external-provider` branch if later approved. |
| Synthetic fixtures are not authoritative clinical content | High | Fixture metadata requires `authoritative=false` and `clinical_use_allowed=false`. | Do not copy licensed SDKI/SLKI/SIKI/NANDA/NOC/NIC content; keep formal registry work separate. |
| Safe trace can regress if new fields are added loosely | High | Trace store allowlist, principal/session trace binding, and canary tests cover current fields. | Require explicit allowlist review and PHI/secret canary tests for every new trace field. |
| Windows symlink fixture tests are platform-dependent | Low | Symlink tests skip when creation privileges are unavailable. | Re-run symlink escape checks under Developer Mode/admin or a platform with symlink support before Sprint C closure. |
